...
from  import , init  # Import the  for colored text

init()  # Initialize the  library for colored text.
...
text_to_encrypt = input(f"{.GREEN}[?] Please Enter your text/message: ")
# Prompt the user to specify the shift length (the key).
key = int(input(f"{.GREEN}[?] Please specify the shift length: "))


# Check if the specified key is within a valid range (0 to 25).
if key > 25 or key < 0:
    # Display an error message if the key is out of range.
    print(f"{.RED}[!] Your shift length should be between 0 and 25 ")
...
print(f"{+] {text_to_encrypt} {.MAGENTA}has been encrypted as {.RED}{encrypted_text}")
